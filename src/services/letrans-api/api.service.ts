...
class ApiService {
    public get = async (path: string): Promise<ApiResponse> => {
        try {
            const response = await axios.get(this.getApiUrl(path), this.getHeaders());
    ...
    public put = async (path: string, data: any = []): Promise<ApiResponse> => {
        try {
            const response = await axios.put(this.getApiUrl(path), data, this.getHeaders());
            return this.getSuccessResponse(response.data);
        } catch (error) {
            return this.getErrorResponse(error.message);
        }
